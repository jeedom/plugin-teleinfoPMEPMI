        ...
        try:
            ...
            while i < self.retry:
                try:
                    ...
                    logging.error('Error on send request to jeedom ' +
                                  str(error)+' retry : '+str(i)+'/'+str(self.retry))
                i = i + 1
            if r.status_code != requests.codes.ok:
                logging.error('Error on send request to jeedom, return code %s' % (
                    str(r.status_code),))
...
class jeedom_serial():
    ...
    def open(self):
        if self.device:
            logging.info("Open serial port on device: " + str(self.device) +
                         ', rate '+str(self.rate)+', timeout : '+str(self.timeout))
        else:
        ...
        try:
            ...
            logging.error("Error: Failed to connect on device " +
                          self.device + " Details : " + str(e))
            return False
    ...
    def write(self, data):
        logging.info("Write data to serial port : " +
                     str(jeedom_utils.ByteToHex(data)))
