...
class LectureFichier(threading.Thread):
    ...
    def __init__(self, chemin_fichier, callback):
        ...
        try:
            ...
            syslog.syslog(
                syslog.LOG_CRIT, "Impossible d'ouvrir le fichier en lecture : " + str(e))
        self._running = True
...
class DecodeCompteurPmePmi():
    ...
    def __init__(self):
        self._CHAR_STX = bytes.fromhex("02")  # to_bytes("02".decode('hex'))
        self._CHAR_ETX = bytes.fromhex("03")  # to_bytes("03".decode('hex'))
        self._CHAR_LF = bytes.fromhex("0A")  # to_bytes("0A".decode('hex'))
        self._CHAR_CR = bytes.fromhex("0D")  # to_bytes("0D".decode('hex'))
        self._CHAR_EOT = bytes.fromhex("04")  # to_bytes("04".decode('hex'))
        self._CHAR_SEPARATEUR = bytes.fromhex(
            "20")  # to_bytes("20".decode('hex'))
    ...
    def __f_traitement_groupe(self):
        ...
        self._tampon_groupe = self._tampon_groupe + self._dernier_octet.decode()

    def __f_interruption(self):
        """ Traitement de l'interruption en cours """
        self._tampon_interruption = self._tampon_interruption + self._dernier_octet.decode()
    ...
    def __f_traitement_fin_de_groupe(self):
        ...
        if (len(self._tampon_groupe) > 3) \
           and (self._checksums_groupes_bons == True):
            ...
            checksum_calcul_resultat = self.calcule_checksum(
                checksum_calcul_chaine)
            # print("Checksum calcule : " + checksum_calcul_resultat)
            if checksum_char == checksum_calcul_resultat:
                premiere_occurence_separateur = checksum_calcul_chaine.find(
                    self._CHAR_SEPARATEUR.decode())
    ...
    def interpreter_trame(self, trame):
            ...
            # Traitement des autres champs
                ...
                if etiquette in self._config_champs:
                    ...
                    if self._config_champs[etiquette][1] == True:
                        liste_vals_numeriques = re.findall(
                            "[-+]?\d*[\.,]\d+|[-+]?\d+", donnee)
                        regexp = re.compile(",")
                        valeur_numerique = regexp.sub(
                            '.', liste_vals_numeriques[0])
                        regexp = re.compile("[-+]?\d*[\.,]\d+|[-+]?\d+")
                        unite = regexp.sub('', donnee, count=1)
                        # print("valeur_numerique (unite) : " + valeur_numerique + " (" + unite + ")")
                        dict_separation_mesures[mesure_en_cours][etiquette] = (
                            valeur_numerique, unite)
                    else:
                        dict_separation_mesures[mesure_en_cours][etiquette] = (
                            donnee, None)
            else:
                print("Attention, cas indetermine !!! etiquette/champ : " +
                      etiquette + " // " + donnee)
                print("Debut du programme sur une trame incomplete ?")
                syslog.syslog(
                    syslog.LOG_WARNING, "Attention, cas indetermine !!! etiquette/champ : " + etiquette + " // " + donnee)
                syslog.syslog(syslog.LOG_WARNING,
                              "Debut du programme sur une trame incomplete ?")
           ...
           (not "ID_COMPTEUR" in dict_separation_mesures):
            ...
            if "TARIFDYN" in dict_separation_mesures["MESURES1"]:
                if dict_separation_mesures["MESURES1"]["TARIFDYN"][0] != "INACTIF":
                    periode_tarifaire_en_cours = dict_separation_mesures["MESURES1"][
                        "PTCOUR1"][0] + "-" + dict_separation_mesures["MESURES1"]["ETATDYN1"][0]
                else:
        ...
        if self.__obtenir_mutex_donnees() == True:
            if interpretation_valide == True and "MESURES1" in dict_separation_mesures:
                ...
                nouveau_tableau_interprete["INDEP_TARIF"]["ID_COMPTEUR"] = copy.deepcopy(
                    dict_separation_mesures["ID_COMPTEUR"])
                nouveau_tableau_interprete["INDEP_TARIF"]["CONTRAT"] = copy.deepcopy(
                    dict_separation_mesures["MESURES1"]["CONTRAT"])
                ...
                for etiquette in dict_separation_mesures["MESURES1"]:
                    ...
                    if etiquette in self._config_champs:
                        ...
                        if self._config_champs[etiquette][0] == True:
                            nouveau_tableau_interprete[periode_tarifaire_en_cours][etiquette] = (
                                donnee, unite)
                            nouveau_tableau_interprete_entree_en_cours = periode_tarifaire_en_cours
                        else:
                            nouveau_tableau_interprete["INDEP_TARIF"][etiquette] = (
                                donnee, unite)
                               ...
                               and (self._epoch_derniere_trame_valide - epoch_ancienne_trame_valide) < 20:
                                if nouveau_tableau_interprete_entree_en_cours in self._dict_interprete:
                                    if etiquette in self._dict_interprete[nouveau_tableau_interprete_entree_en_cours]:
                                        valeur_numerique_float_ancienne = float(
                                            self._dict_interprete[nouveau_tableau_interprete_entree_en_cours][etiquette][0])
                                        valeur_numerique_float_nouvelle = float(
                                            nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours][etiquette][0])
                                        ...
                                        if (math.fabs(valeur_numerique_float_ancienne) <= math.fabs(valeur_numerique_float_nouvelle)) \
                                            ...
                                            if self._config_champs[etiquette][5] == self._zbxtype_int:
                                                nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours][etiquette + "_delta"] = (
                                                    str(int(valeur_numerique_float_nouvelle - valeur_numerique_float_ancienne)), unite)
                                            else:
                                                nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours][etiquette + "_delta"] = (
                                                    str(valeur_numerique_float_nouvelle - valeur_numerique_float_ancienne), unite)
                ...
                if not "CPT_INTERRUPTIONS" in nouveau_tableau_interprete["INDEP_TARIF"]:
                    nouveau_tableau_interprete["INDEP_TARIF"]["CPT_INTERRUPTIONS"] = (
                        "0", None)
                ...
                for nouveau_tableau_interprete_entree_en_cours in nouveau_tableau_interprete:
                    if nouveau_tableau_interprete_entree_en_cours != "INDEP_TARIF":
                        if "EAP_s" in nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours]:
                            total_conso_indep_tarif_s = total_conso_indep_tarif_s + \
                                int(
                                    nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours]["EAP_s"][0])
                        if "EAP_i" in nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours]:
                            total_conso_indep_tarif_i = total_conso_indep_tarif_i + \
                                int(
                                    nouveau_tableau_interprete[nouveau_tableau_interprete_entree_en_cours]["EAP_i"][0])
                if not "CONSO_TOTALE_s" in nouveau_tableau_interprete["INDEP_TARIF"]:
                    nouveau_tableau_interprete["INDEP_TARIF"]["CONSO_TOTALE_s"] = (
                        None, None)
                if not "CONSO_TOTALE_i" in nouveau_tableau_interprete["INDEP_TARIF"]:
                    nouveau_tableau_interprete["INDEP_TARIF"]["CONSO_TOTALE_i"] = (
                        None, None)
                nouveau_tableau_interprete["INDEP_TARIF"]["CONSO_TOTALE_s"] = (
                    str(total_conso_indep_tarif_s), None)
                nouveau_tableau_interprete["INDEP_TARIF"]["CONSO_TOTALE_i"] = (
                    str(total_conso_indep_tarif_i), None)
                ...
                self._dict_interprete = copy.deepcopy(
                    nouveau_tableau_interprete)
                ...
                syslog.syslog(syslog.LOG_WARNING,
                              "Attentions : formalisme de trame invalide !!!!")
            ...
            if interpretation_valide == True:
                ...
                if self._cb_nouvelle_interpretation_tt_interpretation != None:
                    if self.__obtenir_mutex_donnees() == True:
                        # print(self._dict_interprete)
                        self._cb_nouvelle_interpretation_tt_interpretation(
                            self._dict_interprete)
                        self.__relacher_mutex_donnees()
        # Un peu de menage
    ...
    def incrementer_compteur_interruptions(self):
        ...
        if self.__obtenir_mutex_donnees() == True:
            ...
            if not "CPT_INTERRUPTIONS" in self._dict_interprete["INDEP_TARIF"]:
                self._dict_interprete["INDEP_TARIF"]["CPT_INTERRUPTIONS"] = (
                    "0", None)
            CPT_INTERRUPTIONSerruptions = int(
                self._dict_interprete["INDEP_TARIF"]["CPT_INTERRUPTIONS"][0])
            CPT_INTERRUPTIONSerruptions = CPT_INTERRUPTIONSerruptions + 1
            self._dict_interprete["INDEP_TARIF"]["CPT_INTERRUPTIONS"] = (
                str(CPT_INTERRUPTIONSerruptions), None)
            self.__relacher_mutex_donnees()
        ...
        if self.__obtenir_mutex_donnees() == True:
            ...
            if not "CPT_TRAMES_INVALIDES" in self._dict_interprete["INDEP_TARIF"]:
                self._dict_interprete["INDEP_TARIF"]["CPT_TRAMES_INVALIDES"] = (
                    "0", None)
            cpt_trames_invalides = int(
                self._dict_interprete["INDEP_TARIF"]["CPT_TRAMES_INVALIDES"][0])
            cpt_trames_invalides = cpt_trames_invalides + 1
            self._dict_interprete["INDEP_TARIF"]["CPT_TRAMES_INVALIDES"] = (
                str(cpt_trames_invalides), None)
            self.__relacher_mutex_donnees()
    ...
    def __obtenir_mutex_donnees(self):
        ...
        while ((time.time() - epoch_demarrage) < mutex_timeout) and \
            ...
            if self._mutex_donnees_actif == True:
                ...
                syslog.syslog(
                    syslog.LOG_WARNING, "Essai infructueux d'obtention du mutex des donnees interpretees.")
            else:
                mutex_obtenu = True
                self._mutex_donnees_actif = False
                # print("Mutex obtenu")
        ...
        if self.__obtenir_mutex_donnees() == True:
            if ptarif in self._dict_interprete:
                if etiquette in self._dict_interprete[ptarif]:
                    sortie = copy.deepcopy(
                        self._dict_interprete[ptarif][etiquette])
                else:
                    sortie = (None, None)
            else:
                sortie = (None, None)
                    ...
                    if etiquette in self._config_champs:
                        ...
                        if entree_config != {}:
                            if zbx_type != "" and zbx_type != type_donnee_zabbix:
                                ...
                                tableau_config["data"].append(
                                    copy.deepcopy(entree_config))
                            del entree_config
                            entree_config = {}
        self.__relacher_mutex_donnees()
...
class SortieFichier():
    ...
    def __init__(self):
        ...
        try:
            ...
            syslog.syslog(syslog.LOG_WARNING,
                          "Probleme a l'ouverture du fichier de sortie")
